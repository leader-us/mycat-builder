package io.mycat.builder.service;
...
public class CrudGenrator {
  ...
  private ColumnEntity genColumn(String tableName, ResultSet rsColumns) throws SQLException {
    ...
    ColumnEntity columnEntity = new ColumnEntity(tableName, columnName, columnType, columnSize, nullable, comment);
  ...
  public void generator() throws SQLException, IOException, ClassNotFoundException, TemplateException {
    ...
    while (tableRet.next()) {
      ...
      while (rsColumns.next()) {
        ColumnEntity curCol = genColumn(tableName, rsColumns);
    ...
    for (TableEntity tableEntity : tableEntityList) {
      ...
      ;
      mkdirEntity(map, tableEntity.getClassName(),
          new Mytemplate("", "/templates/domain.ftl", "/src/main/java/", ".java"), this.packageName);
      mkdirEntity(map, tableEntity.getClassName(),
          new Mytemplate("Dao", "/templates/dao.ftl", "/src/main/java/", ".java"), this.packageName);
    }
  ...
  private void mkdirEntity(Map<String, Object> hashMap, String entityName, Mytemplate myTemplate, String pkgName)
    ...
    String codePath = System.getProperty("user.dir") + myTemplate.getTargetDir().replace("/", File.separator)
        + packageName.replace(".", File.separator) + File.separator + pkgName;
    ...
    FileUtils.writeStringToFile(
        new File(file.getPath() + File.separator + entityName + myTemplate.getTargetFileSufix()), javacontent, false);
  }
